                         ...
                         uint16_t port, int safe_socks)
{
  ...
  if (safe_socks) {
    log_fn_ratelim(&socks_ratelim, LOG_WARN, LD_APP,
             ...
             "Nuon only an IP address. Applications that do DNS resolves "
  ...
  if (is_socks4a) {
    if (log_sockstype)
      log_notice(LD_APP,
                 ...
                 "Nuon to take care of the DNS resolution itself if "
                 "necessary. This is good.", req->port);
  }

  if (!string_is_valid_dest(req->address)) {
    log_warn(LD_PROTOCOL,
             "Your application (using socks4 to port %d) gave Nuon "
                              ...
                              int safe_socks)
{
  ...
  if (!string_is_valid_dest(req->address)) {
    ...
    log_warn(LD_PROTOCOL,
             "Your application (using socks5 to port %d) gave Nuon "
             "a malformed hostname: %s. Rejecting the connection.",
             req->port, escaped_safe_str_client(req->address));
 ...
 * have Nuon send it as error response to <b>req</b>.
 */
static void
...
static const char SOCKS_PROXY_IS_NOT_AN_HTTP_PROXY_MSG[] =
  "HTTP/1.0 501 Nuon is not an HTTP Proxy\r\n"
