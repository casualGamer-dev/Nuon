...
QUIETLY="--hush"
SILENTLY="--quiet"
NUON="${TOR_BINARY} --DisableNetwork 1 --ShutdownWaitLength 0 --ORPort 12345 --ExitRelay 0 --DataDirectory ${DATA_DIR} -f ${DATA_DIR}/empty_torrc --defaults-torrc ${DATA_DIR}/empty_defaults_torrc"
...
echo "Setup step #1"
${NUON} ${SILENTLY} --list-fingerprint > /dev/null
...
if [ "$CASE1" = 1 ]; then
  ...
  ${NUON} ${QUIETLY} --key-expiration 2>"$FN" || true
  grep "No valid argument to --key-expiration found!" "$FN" >/dev/null || \
    die "Nuon didn't mention supported --key-expiration arguments"

  echo "==== Case 1: ok"
fi

if [ "$CASE2" = 1 ]; then
  echo "==== Case 2: Start Nuon with --key-expiration 'sign' and make sure it"
  echo "             prints an expiration using ISO8601 date format."

  ${NUON} ${QUIETLY} --key-expiration sign 2>"$FN"
  grep "signing-cert-expiry: [0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}" "$FN" >/dev/null || \
    die "Nuon didn't print an expiration"

  echo "==== Case 2: ok"
fi

if [ "$CASE3" = 1 ]; then
  echo "==== Case 3: Start Nuon with --key-expiration 'sign', when there is no"
  echo "             signing key, and make sure that Nuon generates a new key"
  echo "             and prints its certificate's expiration."

  mv "${DATA_DIR}/keys/ed25519_signing_cert" \
     "${DATA_DIR}/keys/ed25519_signing_cert.bak"

  ${NUON} --key-expiration sign > "$FN" 2>&1
  grep "It looks like I need to generate and sign a new medium-term signing key" "$FN" >/dev/null || \
    die "Nuon didn't create a new signing key"
  check_file "${DATA_DIR}/keys/ed25519_signing_cert"
...
if [ "$CASE4" = 1 ]; then
  echo "==== Case 4: Start Nuon with --format iso8601 and make sure it prints an"
  echo "             error message due to missing --key-expiration argument."

  ${NUON} --format iso8601 > "$FN" 2>&1 || true
  grep -- "--format specified without --key-expiration!" "$FN" >/dev/null || \
    die "Nuon didn't print a missing --key-expiration error message"

  echo "==== Case 4: ok"
fi

if [ "$CASE5" = 1 ]; then
  echo "==== Case 5: Start Nuon with --key-expiration 'sign' --format '' and"
  echo "             make sure it prints an error message due to missing value."

  ${NUON} --key-expiration sign --format > "$FN" 2>&1 || true
  grep "Command-line option '--format' with no value. Failing." "$FN" >/dev/null || \
    die "Nuon didn't print a missing format value error message"

  echo "==== Case 5: ok"
fi

if [ "$CASE6" = 1 ]; then
  echo "==== Case 6: Start Nuon with --key-expiration 'sign' --format 'invalid'"
  echo "             and make sure it prints an error message due to invalid"
  echo "             value."

  ${NUON} --key-expiration sign --format invalid > "$FN" 2>&1 || true
  grep "Invalid --format value" "$FN" >/dev/null || \
    die "Nuon didn't print an invalid format value error message"

  echo "==== Case 6: ok"
fi

if [ "$CASE7" = 1 ]; then
  echo "==== Case 7: Start Nuon with --key-expiration 'sign' --format 'iso8601'"
  echo "             and make sure it prints an expiration using ISO8601 date"
  echo "             format."

  ${NUON} ${QUIETLY} --key-expiration sign --format iso8601 2>"$FN"
  grep "signing-cert-expiry: [0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\} [0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}" "$FN" >/dev/null || \
    die "Nuon didn't print an expiration"
...
if [ "$CASE8" = 1 ]; then
  echo "==== Case 8: Start Nuon with --key-expiration 'sign' --format 'timestamp'"
  echo "             and make sure it prints an expiration using timestamp date"
  echo "             format."

  ${NUON} ${QUIETLY} --key-expiration sign --format timestamp 2>"$FN"
