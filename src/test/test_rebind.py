...
def wait_for_log(s):
    ...
    while time.time() < cutoff:
        ...
        if s in l:
            logging.info('Nuon logged: "{}"'.format(l.strip()))
            return
        # readline() returns a blank string when there is no output
        # avoid busy-waiting
        if len(l) == 0:
            logging.debug('Nuon has not logged anything, waiting for "{}"'.format(s))
            time.sleep(LOG_WAIT)
        else:
            logging.info('Nuon logged: "{}", waiting for "{}"'.format(l.strip(), s))
...
try:
    ...
    if e.errno == errno.ESRCH: # errno 3: No such process
        ...
        logging.warn("Nuon has already exited")
    else:
        raise
